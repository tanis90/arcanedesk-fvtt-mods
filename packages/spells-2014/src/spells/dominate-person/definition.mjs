...
const dominatePerson = cleanRoomSpell({
  id: "dominate-person",
  ...
  support: {
    level: "simplified",
    omissions: [
      "目标必须是施法者可见的 humanoid；当前 creature-type/visible predicates 只表达目标契约，由 DM 按现行声明输入政策在施法前确认资格",
      "施法者或其友方正在与目标交战时，首次 Wisdom save 应具有优势；该交战关系与豁免优势由 DM 处理",
      "自动持续时间固定为一分钟；六环为十分钟、七环为一小时、八环或更高为八小时的持续时间由 DM 手动延长来源专注及其 dependent 效果",
      "同位面心灵链接中的命令、目标执行命令，以及施法者用动作取得精确控制并用自身反应迫使目标反应，均由 DM 执行",
    ],
  },
  fragments: [
    graphFragment({
      id: "dominate-person-graph",
      actions: [publicAction("cast", "支配人类 Dominate Person")],
      artifacts: [
        cleanRoomEffect("dominate-person", {
          host: "actor",
          scope: "source-target",
          reapply: "replace",
        ...
        rule({
          ...
          do: [
            operation("apply-artifact", {
              id: "apply-dominate-person",
              artifactId: "dominate-person",
  ...
  accepted: acceptance(L5_WAVE_I2_QA_RECEIPT, [
    "action 对 60 ft 内一个由 DM 明确声明为施法者可见的 humanoid 进行首次 Wisdom save，并只消耗一个五环或更高法术位",
    "首次豁免失败的目标获得具有独立 source-target identity 的 charmed 效果；成功目标与未声明目标不改变；首次豁免成功时，施法者通过受信 Effect UI 结束仍可主动维持的专注（明确 HITL）",
    "受影响目标每次承受至少 1 点实际伤害后进行一次不消耗法术位的普通 Wisdom save；零实际伤害不触发，同一伤害 workflow 对同一来源效果只触发一次",
    "受伤重复豁免成功时只删除本次来源的 Dominate Person 效果，并在最后一个 dependent 消失时结束对应来源专注；其他施法来源不变",
